...
class EncryptionTesterModule(IModule):
    ...
    async def load(self, context):
        ...
        if self.logger:
            self.logger.log(
                "EncryptionTester module loaded",
                tag="tester",
    ...
    async def _run_all_tests(self):
        ...
        test_categories = self._config.get(
            "test_categories",
            ["aes", "rsa", "hmac", "key_management", "integration"]
        )
    ...
    async def _test_rsa(self) -> Dict:
        ...
        try:
            ...
            priv_encrypted = self.encryption.export_private_key(
                private_key, password=b"secret123"
            )
            ...
            imported_priv_enc = self.encryption.import_private_key(
                priv_encrypted, password=b"secret123"
            )
    ...
    async def _test_hmac(self) -> Dict:
        ...
        try:
            ...
            is_valid_tampered = self.encryption.verify_hmac(
                b"tampered message", signature, hmac_key
            )
    ...
    async def _test_integration(self) -> Dict:
        ...
        try:
            ...
            encrypted_session_key = self.encryption.encrypt_with_public(
                session_key, bob_public
            )
            
            # Alice signs the encrypted message
            signature = self.encryption.sign(
                encrypted_message.ciphertext, bob_private
            )
            
            # Bob receives: encrypted_session_key, encrypted_message, signature
            
            # Bob decrypts session key with his private key
            decrypted_session_key = self.encryption.decrypt_with_private(
                encrypted_session_key, bob_private
            )
            assert decrypted_session_key == session_key
            
            # Bob verifies signature
            is_valid = self.encryption.verify(
                encrypted_message.ciphertext, signature, bob_public
            )
            # Note: In real scenario, Alice would sign with her private key
            
            # Bob decrypts message
            decrypted_message = self.encryption.decrypt(
    ...
    def _log_header(self, message: str):
        ...
        if self.logger:
            self.logger.log("", tag="tester")
            self.logger.log("=" * 50, tag="tester")
            self.logger.log(message, tag="tester", text_color=self.colors.BRIGHT_CYAN if self.colors else None)
            self.logger.log("=" * 50, tag="tester")
    ...
    def _log_result(self, category: str, passed: bool):
        ...
        if self.logger:
            ...
            self.logger.log(
                f"\n{'='*30}\n{category}: {status}\n{'='*30}",
                tag="tester",
                text_color=color if self.colors else None
            )
